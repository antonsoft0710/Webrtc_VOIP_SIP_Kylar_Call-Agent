...
const debug = require('debug')('drachtio:kylar');
...
function validateDialedNumber(loggger) {
  const conferenceDid = config.get('conference.did');
  return (req, res, next) => {
    if (conferenceDid !== req.calledNumber) return res.send(404);
    next();
  };
}
